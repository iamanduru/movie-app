$(document).ready(function(){
    ...
    //Get "now playing" dT default.
    //Change results when the genre is clicked on
    function getNowPlayingData() {
        $.getJSON(nowPlayingURL, function(nowPlayingData){
            //console.log(nowPlayingData);
            //We need to add .results coz nowPlayingData is an array.
            for (let i = 0; i<nowPlayingData.results.length; i++){
                ...
                var title = nowPlayingData.results[i].original_tittle;
                ...
                var nowPlayingHTML = '';
                //added in i to nowPlayingHTML without it only details for the movie in the results display in the modal no matter which movie poster you click on.
...
//h1 will change depending on what is clicked. It will display "Now Playing" in this case
                })
    ...
    function getMoviesByGenre(genre_id){
        const getMovieByGenreURL =apiBaseURL + 'genre/' + genre_id + '/movies?api_key=' + apiKey + '&language=en-US&include_adult=false&sort_by=created_at.asc';
        //console.log(getMoviesByGenreURL);

        $.getJSON(getMovieByGenreURL, function(genreData){
            ...
            for(let i = 0; i<genreData.results.length; i++){
                ...
                $.getJSON(thisMovieUrl, function(movieKey){
                    ...
                    var releaseData = genreData.results[i].release_date;
                ...
                //without this line, there is nowhere for the posters and overviews to displays so it doesn't show up
                //$('#movieGenreLabel').html("Now Playing");
                //h1 will change depending on what is clicked.It will display "Now Playing" in this case.
    ...
    //Reset HTML strings to empty to overview with new one!
    var nowPlayingHTML = '';
    var genreHTML = '';
    ...
    //Run the function search movies AFTER an inout has been submitted. Submit form first.
    //Run searchMovies once to add an empty html to movies-grid using .html().Then overwrite it with the new html using .append(). You need to use .append() to overwrite or all the images will display on top of each other.

    var searchTerm = '';
    searchMovies();
    //This references the entire search form
